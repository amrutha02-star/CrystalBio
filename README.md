...
- Admin Reports should follow the same one-flow rule: choose report date range, choose who the report is for, then tap one Generate report button, with a short helper note reinforcing that PDF uses the same summary-first flow. Admin must support Whole office, role-wise, and individual agent report generation without duplicate period/generate controls. Report output should be summary-first with simple charts, `Needs office action`, person-wise cards, and collapsible detail sections for attendance, sales/service visits, leave, missing information, and office actions. PDF exports should use the same structure: first-page summary, detailed appendix after.
