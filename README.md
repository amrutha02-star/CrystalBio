...
- **Agent home** should match the approved admin polish: light typography, compact sage/olive cards, simple full-screen mobile spacing, and no heavy dashboard styling. Recent visits must use the same clean submitted-work pattern as admin: customer name first, agent name second, lighter type/next-action details, and no bold raw audit-code-first rows.

- **Admin overview** is the useful home dashboard: show today's field counts and office-action needs without a submitted-work list. The four top status cards must be real tappable/expandable controls, not dummy tiles. Submitted Sales/Service work belongs under Field Entry, not as a card on the admin home dashboard and not repeated under Agents. Keep the good-morning name greeting in the header; logout belongs inside Profile, not in the overview/header. Do not dump agent names here.
- **Agents** is the admin people/team visibility screen, not another submitted-form list. It should show agent details, attendance, check-in/check-out time, selected work mode, last active status, and compact activity counts. Do not repeat the Sales/Service form rows here. Agent detail may include a button to view that agent’s entries, but it should hand off to Field Entry rather than duplicating forms inside Agents.
- **Field entry** is separate from Agents and is the submitted-form/entry lookup owner. It is the admin path for quick Sales/Service entry plus fast saved-work lookup. Keep simple Sales/Service choices first, then a home-screen-style entries list with My entries / All entries filters and search.
- **Field entry saved visibility:** entries saved from Admin Field Entry must be visible directly on the Field Entry screen after refresh/re-login under My entries for that same admin. All entries may show the whole team in the same simple card pattern for quick lookup; detailed submitted-form review belongs under Field Entry, not repeated under Agents.
...
- **Profile** owns controlled access visibility, password reset/setup links, deactivate/reactivate rules, logout, and a compact active/setup-pending/inactive seat summary. For this pilot, users are pre-created with unique passwords outside the frontend; public signup and automatic email sending are not shown in the frontend.
...
- Visits / My Entries should keep search, New Sales, New Service, and Previous entries in one simple screen. Previous entries should clearly say they can be tapped to continue updates/follow-ups, not look like static history.
- Agent report generation is currently a visual preview flow only. Keep the preview label visible until the user approves connecting this page to backend report generation.
- **Admin Reports** should answer “what business happened?” first: compact summary, office action only when present, and attendance summary counts only. Do not show a Submitted forms/customer entry list in Reports because submitted-form review belongs in Field Entry. Do not dump all employees, add `View team names`/person-wise lists, or show explanatory `Open Agents` prompts in Reports; person/team status belongs in Agents, while submitted-form lookup belongs in Field Entry.
- Sales and Service visit pages must keep the existing app structure/navigation and the progressive Step 1 / Step 2 / Step 3 format. Enhancements should happen inside that structure: automatic agent/time/GPS reassurance, quick contact/equipment capture in Step 1, and simple office-action markers in Step 3. Do not add an extra assistant/shortcut card above Step 1; search and previous entries belong on the Visits page.
- **Admin detail navigation:** safe list-to-detail journeys should support normal browser Back / Android Back / iOS edge-swipe by using real browser history, not broad custom swipe gestures. Apply this only where returning from a detail view to its list makes sense: submitted entries from Field Entry/Reports, Approvals detail, Agents person detail, and Profile person detail. Do not add custom swipe behavior to forms, photo upload/preview, long Sales/Service entry screens, filters, or bottom navigation.
...
- `npm run clean:pilot-data -- --bloom-only --write` clears only Bloom-created QA activity after a pre-clean backup. Full activity/session reset is guarded and requires explicit reset approval.
