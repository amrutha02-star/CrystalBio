...
- **Admin overview** is the useful home dashboard: show today's field counts plus latest submitted Sales/Service work and office-action needs. The four top status cards must be real tappable/expandable controls, not dummy tiles. Latest submitted work should show customer name and agent name in light typography, not bold audit IDs. Keep the good-morning name greeting in the header; logout belongs inside Profile, not in the overview/header. Do not dump agent names here.
- **Agents** is the owner/admin submitted-entry review screen first: show the compact checked-in / need-check / follow-up status at the top once, then recent 5 Sales/Service entries, allow View all, filter by agent/type, and open the actual filled form details read-only. Entry cards must follow the light Home/Field Entry pattern: customer name first, agent name second, no bold audit-code-first rows. Do not add a second full-team/person-name dump under the entries list. It must not look like report generation and should not carry report date-range/export controls.
- **Field entry** is separate from Agents. It is the admin path for quick Sales/Service entry and fast submitted-work lookup. Keep simple Sales/Service choices first, then a home-screen-style entries list with My entries / All entries filters and search.
- **Field entry saved visibility:** entries saved from Admin Field Entry must be visible directly on the Field Entry screen after refresh/re-login under My entries for that same admin. All entries may show the whole team in the same simple card pattern for quick lookup; detailed person/team review still belongs under Agents.
...
- **Admin Reports** should answer “what business happened?” first: compact summary, office action only when present, and attendance summary counts only. Do not show a Submitted forms/customer entry list in Reports because that same submitted-work review already belongs in Agents. Do not dump all employees, add `View team names`/person-wise lists, or show explanatory `Open Agents` prompts in Reports; person-level/submitted-form review belongs in Agents only.
