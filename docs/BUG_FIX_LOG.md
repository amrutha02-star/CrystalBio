...
- Bloom retest: Source retest passed, then live retest passed after deploy. Evidence: `docs/qa-runs/BACKEND_AUDIT_HARDENING_LIVE_RETEST_BLOOM_2026-07-03.md`.
- Live deploy: Deployed after Amrutha approval on 2026-07-03. Pre-deploy backup `/var/lib/crystalbio/backups/crystalbio-db.json.2026-07-03T09-39-41-143Z.bak`; `npm run backend:build` passed; `systemctl restart crystalbio-backend`; live API health OK; live app version remains `20260703033332`.
- Final acceptance: Accepted by Periwinkle after Bloom live retest passed.
- Notes: Periwinkle restored backend process ownership to managed `crystalbio-backend.service` after Bloom cleanup. Wider mobile GPS capture bug remains open and is not closed by coordinate validation alone.
