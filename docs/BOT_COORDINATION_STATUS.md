...
- Backend audit fixes 024-027 are now **deployed live and waiting for Bloom live retest/acceptance**: wrong Sales/Service role creates are blocked in source, impossible GPS coordinates are rejected in source, password reset/setup clears stale sessions, duplicate-visit matching keeps different-time visits separate, setup/session tokens use crypto-random values, and Nodemailer was upgraded to clear the production dependency audit. Bloom source retest passed on 2026-07-03 (`docs/qa-runs/BACKEND_AUDIT_HARDENING_SOURCE_RETEST_BLOOM_2026-07-03.md`). Periwinkle deployed the backend after Amrutha approval, with pre-deploy backup `/var/lib/crystalbio/backups/crystalbio-db.json.2026-07-03T09-39-41-143Z.bak`, backend restart, live API health OK, service active, and live app version still `20260703033332`. Bloom live retest has been assigned; not accepted yet.
