...
- 2026-07-08 Periwinkle live audit for Amrutha: backend/admin reports are recording real agent activity. BUG-20260708-028 is now **deployed live and Periwinkle live-checked**: Admin Reports still returns 90 Sales + 35 Service visit details for 2026, and Admin Field Entry `All entries` now returns 125 lightweight Sales/Service entries instead of only 30. Live browser showed `10 of 125 shown`, Show all expanded to `125 of 125 shown`, search found older Dr. Swati entries, and console had 0 errors. Waiting for Bloom retest/owner acceptance; no QA records were created.
...
- Bloom-created QA records from the July 2, July 3, July 4, July 5, July 6, and July 7 runs were cleaned through Bloom-only backup/write/restart/verification paths; no real-user cleanup was done by guess.
- BUG-20260702-023 location/GPS remains the main active product issue. The bad Chrome-specific copy was removed live. Bloom's 2026-07-07 21:03 IST nightly stabilization run still saw `Location could not be captured` in the live browser Sales path. Backend GPS validation works and typed details were preserved, but the issue is **not accepted** because Android/iPhone/Samsung/iPhone Chrome/Safari capture paths are not fully verified and 30-minute recent-GPS reuse needs owner review.
