...
- Latest Bloom 2:30 AM retest report is `docs/qa-runs/QA_RUN_BLOOM_2026-07-07_230_RETEST.md`: live app/API were up; Admin Field Entry/API refresh evidence passed again; GPS/location capture still failed in the live mobile Sales path, while typed form details were preserved and no Chrome-only copy appeared. Backend GPS guards passed, saved-login/session checks passed in cron/API context, agent Attendance/Visit/Combined PDF downloads passed again, and Bloom-only retest records were cleaned after dry-run/backup/write/restart.
