...
- BUG-20260702-023 location capture issue is newly under Periwinkle review after Amrutha reported a Sales agent cannot add location. Initial check: live API is OK, backend requires GPS by design, Bloom evidence shows saves work when GPS is allowed, and live logs show repeated `Allow location permission to save this field update.` on Android Chrome. Treat as a real-user UX/recovery issue to classify before Iris fixes.
