...
- Amrutha caught extra Bloom QA temp profiles still visible in Admin Agents. Correct rule recorded now: Bloom QA must use only the standard two accounts (`bloom.admin@crystalbio.in` for admin-side testing and `bloom.agent@crystalbio.in` for agent-side testing). Periwinkle removed the 3 inactive Bloom-created backend live-retest temp profiles on 2026-07-13 after backup/dry-run; no real user records and no Sales/Service/attendance/leave records were removed. Live Admin Agents now shows only the 2 standard Bloom accounts.
