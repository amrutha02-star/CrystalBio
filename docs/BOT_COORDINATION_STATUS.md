...
- Backend audit fixes 024-027 are now **source-fixed and built/tested locally only**: wrong Sales/Service role creates are blocked, impossible GPS coordinates are rejected, password reset/setup clears stale sessions, duplicate-visit matching now keeps different-time visits separate, setup/session tokens use crypto-random values, and Nodemailer was upgraded to clear the production dependency audit. Checks passed so far: targeted backend/API tests 40/40, full test suite 115/115, `npm run backend:build`, `npm run build`, and production `npm audit --omit=dev` 0 vulnerabilities. Not live-deployed or accepted yet; needs Bloom retest/safe deploy approval.
