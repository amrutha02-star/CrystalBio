...
- Backend audit fixes 024-027 are now **source-fixed and Bloom source-retested only**: wrong Sales/Service role creates are blocked, impossible GPS coordinates are rejected, password reset/setup clears stale sessions, duplicate-visit matching now keeps different-time visits separate, setup/session tokens use crypto-random values, and Nodemailer was upgraded to clear the production dependency audit. Bloom source retest passed on 2026-07-03 (`docs/qa-runs/BACKEND_AUDIT_HARDENING_SOURCE_RETEST_BLOOM_2026-07-03.md`): targeted backend/API tests 40/40, full test suite 115/115, `npm run backend:build`, `npm run build`, production `npm audit --omit=dev` 0 vulnerabilities, and targeted API probes all passed. Not live-deployed or accepted yet; needs safe deploy/live retest approval.
