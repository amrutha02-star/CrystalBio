...
Use two agents with separate responsibilities:

1. **Testing Agent** — checks the app like a real user and documents everything on GitHub.
2. **Bug-Fixer Agent** — fixes confirmed bugs, but never verifies its own work.

A bug is only considered fixed after the Testing Agent retests it and records evidence.
