...
- QA/testing must use the standard two Bloom accounts only: `bloom.admin@crystalbio.in` for admin-side testing and `bloom.agent@crystalbio.in` for agent-side testing. Do not create extra Bloom/temp user profiles for normal QA. If a one-off backend/security test absolutely requires a temporary Bloom user, it must be documented, deactivated/removed from normal admin views after evidence, and not left visible in the live team list.
...
- When Amrutha asks for user journey testing, do not return a screen inventory, raw QA log, or long PDF narrative. User journey testing must follow real end-to-end business stories from user intent → action → save/submit → refresh/reopen → admin visibility/reporting → failure/edge case → clear pass/fail, preferably in a compact table. Each row must state the journey, scenario, expected result, actual result, status, evidence, and next action.
...
- If Amrutha asks to keep something noted, do not rely on chat memory. Record the instruction in `AGENTS.md` or the relevant linked `docs/` file, then refer to the doc in future replies/work.
- For location/GPS bugs, do not patch with browser-specific wording or assume only Chrome is affected. Treat missing permission prompts as a cross-phone/browser capture problem covering Android Chrome, Samsung Browser, iPhone Safari, iPhone Chrome, and home-screen/PWA use. Diagnose and test the capture flow before any live deploy; preserve typed form data and the GPS requirement.
...
- Keep the repository and GitHub clean as a basic finishing step: start and finish each CrystalBio task with `git status --short --branch`; commit approved source/doc/evidence changes in focused commits; push commits to the GitHub upstream when network/auth allows; move generated screenshots/logs/temp outputs out of the repo or into ignored/evidence paths; and do not leave important deployed changes only in the working tree or only on the local machine. If GitHub is not current, say that plainly.
